...
class iSDK
{
     ...
     * @throws iSDKException
     */
    $app->vendorCon($name, $user, $pass, $key, $dbOn)
    {
        $this->debug = (($key == 'on' || $key == 'off' || $key == 'kill' || $key == 'throw') ? $key : $dbOn);
     ...
     * @throws iSDKException
     */
    $app->methodCaller($service, $callArray)
    {
        /* Set up the call */
     ...
     * @return array
     */
    $app->getAffiliatesByProgram($programId)
    {
        $carray = array(
     ...
     * @return array
     */
    $app->getProgramsForAffiliate($affiliateId)
    {
        $carray = array(
     ...
     * @return array
     */
    $app->affClawbacks($affId, $startDate, $endDate)
    {
        $carray = array(
     ...
     * @return array
     */
    $app->affCommissions($affId, $startDate, $endDate)
    {
        $carray = array(
            php_xmlrpc_encode((int)$affId),
            php_xmlrpc_encode($startDate, array('auto_dates')),
     ...
     * @return array
     */
    $app->affPayouts($affId, $startDate, $endDate)
    {
        $carray = array(
            php_xmlrpc_encode((int)$affId),
            php_xmlrpc_encode($startDate, array('auto_dates')),
     ...
     * @return array
     */
    $app->affSummary($affList, $startDate, $endDate)
    {
        $carray = array(
            php_xmlrpc_encode($affList),
            php_xmlrpc_encode($startDate, array('auto_dates')),
     ...
     * @return bool
     */
    $app->resumeCampaignForContact($cid, $sequenceId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$cid),
     ...
     * @return array
     */
    $app->getNextCampaignStep($cid, $campId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$cid),
            php_xmlrpc_encode((int)$campId));
     ...
     * @return array
     */
    $app->getCampaigneeStepDetails($cid, $stepId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$cid),
     ...
     * @return int
     */
    $app->rescheduleCampaignStep($cidList, $campId)
    {
        $carray = array(
     ...
     * @return int|mixed|string
     */
    $app->applyActivityHistoryTemplate($contactId, $historyId, $userId)
    {
        $carray = array(
     ...
     * @return array
     */
    $app->getActivityHistoryTemplateMap()
    {
        $carray = array();
     ...
     * @return int
     */
    $app->addWithDupCheck($cMap, $checkType)
    {
        $carray = array(
            php_xmlrpc_encode($cMap, array('auto_dates')),
     ...
     * @return string
     */
    $app->requestCcSubmissionToken($contactId, $successUrl, $failureUrl)
    {
        $carray = array(
            php_xmlrpc_encode((int)$contactId),
     ...
     * @return int|mixed|string
     */
    $app->dsGetSetting($module, $setting)
    {
        $carray = array(
     ...
     * @return int
     */
    $app->dsUpdateWithImage($tName, $id, $iMap)
    {
        $carray = array(
            php_xmlrpc_encode($tName),
            php_xmlrpc_encode((int)$id),
     ...
     * @return array
     */
    $app->dsFind($tName, $limit, $page, $field, $value, $rFields)
    {
        $carray = array(
            php_xmlrpc_encode($tName),
     ...
     * @return array
     */
    $app->dsQuery($tName, $limit, $page, $query, $rFields)
    {
        $carray = array(
            php_xmlrpc_encode($tName),
            php_xmlrpc_encode((int)$limit),
     ...
     * @return array
     */
    $app->dsQueryOrderBy($tName, $limit, $page, $query, $rFields, $orderByField, $ascending = TRUE)
    {
        $carray = array(
            php_xmlrpc_encode($tName),
            php_xmlrpc_encode((int)$limit),
     ...
     * @return int
     */
    $app->addCustomField($context, $displayName, $dataType, $headerID)
    {
        $carray = array(
     ...
     * @return int
     */
    $app->updateCustomField($fieldId, $fieldValues)
    {
        $carray = array(
     ...
     * @return int
     */
    $app->addFreeTrial($name, $description, $freeTrialDays, $hidePrice, $subscriptionPlanId)
    {
        $carray = array(
     ...
     * @return int
     */
    $app->addOrderTotalDiscount($name, $description, $applyDiscountToCommission, $percentOrAmt, $amt, $payType)
    {
        $carray = array(
            php_xmlrpc_encode((string)$name),
            php_xmlrpc_encode((string)$description),
     ...
     * @return int
     */
    $app->addCategoryDiscount($name, $description, $applyDiscountToCommission, $amt)
    {
        $carray = array(
            php_xmlrpc_encode((string)$name),
            php_xmlrpc_encode((string)$description),
     ...
     * @return int
     */
    $app->addCategoryAssignmentToCategoryDiscount($categoryDiscountId, $productCategoryId)
    {
        $carray = array(
     ...
     * @return array
     */
    $app->getCategoryAssignmentsForCategoryDiscount($id)
    {
        $carray = array(
            php_xmlrpc_encode((int)$id));
     ...
     * @return int
     */
    $app->addProductTotalDiscount($name, $description, $applyDiscountToCommission, $productId, $percentOrAmt, $amt)
    {
        $carray = array(
            php_xmlrpc_encode((string)$name),
            php_xmlrpc_encode((string)$description),
     ...
     * @return array
     */
    $app->getProductTotalDiscount($id)
    {
        $carray = array(
            php_xmlrpc_encode((int)$id));
     ...
     * @return int
     */
    $app->addShippingTotalDiscount($name, $description, $applyDiscountToCommission, $percentOrAmt, $amt)
    {
        $carray = array(
            php_xmlrpc_encode((string)$name),
            php_xmlrpc_encode((string)$description),
     ...
     * @return array
     */
    $app->getShippingTotalDiscount($id)
    {
        $carray = array(
            php_xmlrpc_encode((int)$id));
     ...
     * @return bool
     */
    $app->attachEmail($cId, $fromName, $fromAddress, $toAddress, $ccAddresses,
     ...
     * @return array
     */
    $app->getAvailableMergeFields($mergeContext)
    {
        $carray = array(
     ...
     * @return bool
     */
    $app->sendEmail($conList, $fromAddress, $toAddress, $ccAddresses, $bccAddresses, $contentType, $subject, $htmlBody, $txtBody)
    {
        $carray = array(
     ...
     * @return bool
     */
    $app->sendTemplate($conList, $template)
    {
        $carray = array(
            php_xmlrpc_encode($conList),
     ...
     * @return int
     */
    $app->createEmailTemplate($title, $userID, $fromAddress, $toAddress, $ccAddresses, $bccAddresses, $contentType, $subject, $htmlBody,
                                        $txtBody)
    {
        $carray = array(
     ...
     * @return int
     */
    $app->addEmailTemplate($title, $category, $fromAddress, $toAddress, $ccAddresses, $bccAddresses, $subject, $txtBody, $htmlBody, $contentType, $mergeContext)
    {
        $carray = array(
            php_xmlrpc_encode($title),
            php_xmlrpc_encode($category),
     ...
     * @return array
     */
    $app->getEmailTemplate($templateId)
    {
        $carray = array(
     ...
     * @return bool
     */
    $app->updateEmailTemplate($templateID, $title, $categories, $fromAddress, $toAddress, $ccAddress, $bccAddress, $subject, $textBody, $htmlBody, $contentType, $mergeContext)
    {
        $carray = array(
            php_xmlrpc_encode((int)$templateID),
            php_xmlrpc_encode($title),
     ...
     * @return bool
     */
    $app->optIn($email, $reason = 'Contact Was Opted In through the API')
    {
        $carray = array(
            php_xmlrpc_encode($email),
     ...
     * @return bool
     */
    $app->optOut($email, $reason = 'Contact Was Opted Out through the API')
    {
        $carray = array(
            php_xmlrpc_encode($email),
            php_xmlrpc_encode($reason));
     ...
     * @return int|mixed|string
     */
    $app->uploadFile($fileName, $base64Enc, $cid = 0)
    {
        $result = 0;
        if ($cid == 0) {
            $carray = array(
     ...
     * @return bool
     */
    $app->replaceFile($fileID, $base64Enc)
    {
        $carray = array(
            php_xmlrpc_encode((int)$fileID),
            php_xmlrpc_encode($base64Enc));
     ...
     * @return bool
     */
    $app->renameFile($fileID, $fileName)
    {
        $carray = array(
            php_xmlrpc_encode((int)$fileID),
            php_xmlrpc_encode($fileName));
     ...
     * @return array
     */
    $app->achieveGoal($integration, $callName, $contactId)
    {
        $carray = array(
     ...
     * @return bool
     */
    $app->setInvoiceSyncStatus($Id, $syncStatus)
    {
        $carray = array(
            php_xmlrpc_encode((int)$Id),
     ...
     * @return bool
     */
    $app->setPaymentSyncStatus($Id, $Status)
    {
        $carray = array(
            php_xmlrpc_encode((int)$Id),
     ...
     * @return int
     */
    $app->manualPmt($invId, $amt, $payDate, $payType, $payDesc, $bypassComm)
    {
        $carray = array(
     ...
     * @return bool
     */
    $app->commOverride($invId, $affId, $prodId, $percentage, $amt, $payType, $desc, $date)
    {
        $carray = array(
            php_xmlrpc_encode((int)$invId),
            php_xmlrpc_encode((int)$affId),
     ...
     * @return bool
     */
    $app->addOrderItem($ordId, $prodId, $type, $price, $qty, $desc, $notes)
    {
        $carray = array(
     ...
     * @return bool
     */
    $app->payPlan($ordId, $aCharge, $ccId, $merchId, $retry, $retryAmt, $initialPmt, $initialPmtDate, $planStartDate, $numPmts, $pmtDays)
    {
        $carray = array(
            php_xmlrpc_encode((int)$ordId),
     ...
     * @return int
     */
    $app->addRecurring($cid, $allowDup, $progId, $merchId, $ccId, $affId, $daysToCharge)
    {
        $carray = array(
            php_xmlrpc_encode((int)$cid),
     ...
     * @return int
     */
    $app->addRecurringAdv($cid, $allowDup, $progId, $qty, $price, $allowTax, $merchId, $ccId, $affId, $daysToCharge)
    {
        $carray = array(
            php_xmlrpc_encode((int)$cid),
            php_xmlrpc_encode($allowDup),
     ...
     * @return array
     */
    $app->chargeInvoice($invId, $notes, $ccId, $merchId, $bypassComm)
    {
        $carray = array(
            php_xmlrpc_encode((int)$invId),
            php_xmlrpc_encode($notes),
     ...
     * @return int
     */
    $app->blankOrder($conId, $desc, $oDate, $leadAff, $saleAff)
    {
        $carray = array(
     ...
     * @return bool
     */
    $app->updateSubscriptionNextBillDate($subscriptionId, $nextBillDate)
    {
        $carray = array(
     ...
     * @return bool|string
     */
    $app->infuDate($dateStr, $dateFrmt = 'US')
    {
     ...
     * @return array
     */
    $app->placeOrder($contactId, $creditCardId, $payPlanId, $productIds, $subscriptionIds, $processSpecials, $promoCodes, $leadAff = 0, $saleAff = 0)
    {
        $carray = array(
            php_xmlrpc_encode((int)$contactId),
     ...
     * @return bool
     */
    $app->incrementInventory($productId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$productId));
     ...
     * @return bool
     */
    $app->increaseInventory($productId, $quantity)
    {
        $carray = array(
            php_xmlrpc_encode((int)$productId),
     ...
     * @return bool
     */
    $app->decreaseInventory($productId, $quantity)
    {
        $carray = array(
            php_xmlrpc_encode((int)$productId),
            php_xmlrpc_encode((int)$quantity));
     ...
     * @return bool
     */
    $app->deactivateCreditCard($creditCardId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$creditCardId));
     ...
     * @return array
     */
    $app->savedSearchAllFields($savedSearchId, $userId, $page)
    {
        $carray = array(
     ...
     * @return array
     */
    $app->savedSearch($savedSearchId, $userId, $page, $fields)
    {
        $carray = array(
            php_xmlrpc_encode((int)$savedSearchId),
            php_xmlrpc_encode((int)$userId),
     ...
     * @return array
     */
    $app->getAvailableFields($savedSearchId, $userId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$savedSearchId),
            php_xmlrpc_encode((int)$userId));
     ...
     * @return array
     */
    $app->getDefaultQuickSearch($userId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$userId));
     ...
     * @return array
     */
    $app->quickSearch($quickSearchType, $userId, $filterData, $page, $limit)
    {
        $carray = array(
     ...
     * @return bool
     */
    $app->addMoveNotes($ticketList, $moveNotes, $moveToStageId, $notifyIds)
    {
        $carray = array(
     ...
     * @return bool
     */
    $app->moveTicketStage($ticketID, $ticketStage, $moveNotes, $notifyIds)
    {
        $carray = array(
     ...
     * @return array
     */
    $app->getFlatRateShippingOption($optionId)
    {
        $carray = array(
     ...
     * @return array
     */
    $app->getOrderTotalShippingOption($optionId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$optionId));
     ...
     * @return array
     */
    $app->getOrderTotalShippingRanges($optionId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$optionId));
     ...
     * @return array
     */
    $app->getProductBasedShippingOption($optionId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$optionId));
     ...
     * @return array
     */
    $app->getProductShippingPricesForProductShippingOption($optionId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$optionId));
     ...
     * @return array
     */
    $app->getOrderQuantityShippingOption($optionId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$optionId));
     ...
     * @return array
     */
    $app->getWeightBasedShippingOption($optionId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$optionId));
     ...
     * @return array
     */
    $app->getWeightBasedShippingRanges($optionId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$optionId));
     ...
     * @return array
     */
    $app->getUpsShippingOption($optionId)
    {
        $carray = array(
            php_xmlrpc_encode((int)$optionId));
     ...
     * @return string
     */
    $app->getWebFormHtml($webFormId = 0)
    {
        $carray = array(
